...
update pro_account set pro_account.pro_description = ('エアコン分解洗浄、洗濯機分解清掃、換気フード分解清掃、マットレスケア、水道＆ボイラー配管洗浄、分配器交換などを専門とするホームクリーニング専門業者「イソルホームケア」でございます。 専門性・丁寧さ・高品質なサービス・そして納得のいく価格をモットーに、常にお客様のご満足を最優先に考え、「自分の家族が使うつもりで」誠実かつ親切に、心を込めて作業させていただきます。 お客様、ご希望の日程やお時間帯がございましたら、お気軽にお知らせくださいませ。できる限りご都合に合わせて調整いたします。 作業中はメッセージの確認が難しい場合がございますので、恐れ入りますがお電話でのお問い合わせをお願いいたします。') where pro_pk = 4;
